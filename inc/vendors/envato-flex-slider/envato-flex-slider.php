...
function efs_get_slider(){
        ...
        if (have_posts()) :     
                ...
                $slide_link = slider_link_get_meta_box_data(get_the_ID());
                        ...
                        $slider.= '<li class="orbit-slide is-active"><div class="orbit-slide-number"><span>'.intval($x).'</span> of <span>'.intval($count).'</span></div><a href="'.esc_url($slide_link).'">'.$img.'</a><figcaption class="orbit-caption">'.esc_html($caption).'</figcaption></li>';
                        $x++;
                endwhile;
